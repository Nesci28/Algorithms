...
function getInfo(arr, num) {
  const name = arr[0].match("<(.*)>")
  const address = arr[0].replace('<' + name[1] + '>', '').replace(num, '').replace(/[^a-z0-9-\.]/gmi, " ").trim().replace(/ +(?= )/g,'')
  return `Phone => ${num}, Name => ${name[1]}, Address => ${address}`
}

function phone(str, num) {
  const arr = str.split('\n').filter(ele => ele.includes(num))
  return (arr.length == 0) ? `Error => Not found: ${num}` : (arr.length > 1) ? `Error => Too many people: ${num}` : getInfo(arr, num)
}