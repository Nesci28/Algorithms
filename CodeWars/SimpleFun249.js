/* https://www.codewars.com/kata/590c3b1ff06c49a024000001
...
When subtracting 1234 from 2024, we actually consider the following pairs (going from right to left): 4 and 4; 12 (10 was borrowed from 0, temporarily making it equal to -1, and added to 2) and 3; 9 (again, 10 was borrowed from 2) and 2; 1 and 1.

*/

function subtractionByRegrouping(total, minus) {
  var res = []
  total = total.toString().split('').reverse()
  minus = minus.toString().split('').reverse()
  total.map((x, i) => {
    if (x - minus[i] >= 0) {
      res.push(parseInt(x))
    } else {
      total[i+1] = parseInt(total[i+1])-1
      total[i] = parseInt(total[i])+10
      res.push(parseInt(total[i]))
    }
  })
  return res
}